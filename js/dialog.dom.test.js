...
describe("addTaskDialog DOM Component", () => {
  beforeEach(() => {
    ...
    mockTaskService.addTask.mockResolvedValue(undefined);
  ...
  test("should render dialog initial state", () => {
    ...
    expect(
      scope.getByRole("heading", { name: "Skapa uppgift" })
    ).toBeInTheDocument();
    ...
    expect(
      scope.getByPlaceholderText("Beskrivning av uppgiften...")
    ).toBeInTheDocument();
  ...
  test("edit mode populates values", () => {
    ...
    expect(
      scope.getByRole("heading", { name: "Redigera uppgift" })
    ).toBeInTheDocument();
  ...
  test("create calls service.addTask", async () => {
    const overlay = addTaskDialog(mockTaskService);
    document.body.appendChild(overlay);
  ...
  test("edit calls service.updateTask", async () => {
    const mockTask = {
      id: "1",
      title: "Old",
      description: "",
